...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < goal and opponent_score < goal :
        ...
        if who == 0:
            ...
            score = score + take_turn(num_rolls, opponent_score, select_dice(score, opponent_score))
            who = other(who)
            
        else:
            
            num_rolls = strategy1(opponent_score,score)
            opponent_score = opponent_score + take_turn(num_rolls, score, select_dice(opponent_score, score))
            who = other(who)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    while current_dice <= 10:
        current_score = make_averaged(roll_dice, num_samples=1000)(current_dice,dice)
        if current_score > maxscore:
            maxscore = current_score
            max_dice = current_dice
        print(current_dice , "dice scores", maxscore , "on average")
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    if score + (1 + max(opponent_score % 10, opponent_score//10 )) ==  opponent_score/2:
        return 0
    elif (score + (1 + max(opponent_score % 10, opponent_score//10 )))/2 ==  opponent_score:
        return BASELINE_NUM_ROLLS
    elif 1 + max(opponent_score % 10, opponent_score//10 ) >= BACON_MARGIN:
        return 0
    else:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    My personal strategy is that implemented on the combination use of 
    swap strategy and bacon strategy, with a more specific circumstances with the gaps in points
    and how close we are to the goals.
    ...
    if score + (1 + max(opponent_score % 10, opponent_score//10 )) >= 100 and score + (1 + max(opponent_score % 10, opponent_score//10 )) != opponent_score*2:
        return 0
    
    if score + (1 + max(opponent_score % 10, opponent_score//10 )) ==  opponent_score/2:
        return 0
    
    if score + 1 ==  opponent_score/2 and score > 30:
        return 10
    
    if (score + (1 + max(opponent_score % 10, opponent_score//10 )) + opponent_score) %7 == 0:
        return 0
